...
export default function ManageUsers({ userRole, title }) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div className='flex items-center space-x-3'>
            ...
            <div>
                ...
                <p className="text-gray-600 mt-1">Add, view, and manage accounts for your hospital.</p>
            </div>
        </div>
        <button
              ...
              users.map(user => (
                <tr key={user.id} className="bg-white border-b hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                    {user.full_name}
                    <p className="md:hidden text-xs text-gray-500 font-normal">{user.email}</p>
                  </td>
...
ManageUsers.propTypes = {
  userRole: PropTypes.oneOf(['doctor', 'nurse', 'medical_shop']).isRequired,
